...
firstHalf a
    | even (length a) = take (div (length a) 2) a
    | otherwise = take (div (length a) 2 + 1) a
...
capitalizeFirst a = toUpper (head a) : tail a
...
joinToLength x list = [a ++ b| a <- list, b <-list, length(a ++ b) == x ]
...
sumRights [] = 0
sumRights (list) = head (rights (map mapIt list) ) + (sumRights (tail (map mapIt list)))

mapIt :: Either a Int -> Either a Int
mapIt (Right a) = (Right a)
mapIt (Left b) = (Right 0)
...
multiApp f gs x = f (applyFunctions gs x)

applyFunctions :: [c->a] -> c -> [a]
applyFunctions [] x = []
applyFunctions gs x = head gs x : applyFunctions (tail gs) x
...
interpreter [] = []
interpreter commands = interpreter_ commands (0,0) 

interpreter_ :: [String] -> (Int, Int) -> [String]
interpreter_ [] (x,y)= []
interpreter_ commands (x,y) 
    | head commands == "up" = interpreter_ (tail commands) (x,y+1)
    | head commands == "down" = interpreter_ (tail commands) (x,y-1)
    | head commands == "right" = interpreter_ (tail commands) (x+1,y)
    | head commands == "left" = interpreter_ (tail commands) (x-1,y)
    | otherwise = printing commands (x,y) : interpreter_ (tail commands) (x,y)


printing :: [String] -> (Int, Int) -> String
printing commands (x,y) 
    | head commands == "printX" = show x
    | otherwise  = show y
